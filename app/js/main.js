...
riot.tag2('app-inventory', '<div if="{!hasItems()}"> Your inventory is empty. Use the scanner function <i class="scan"></i> , to find items in VIRTTRUHE Codes. </div> <ul class="items"> <li each="{items()}" class="{selected:isSelected(this)}" onclick="{select}"> <img riot-src="{getItemImageSrc(this)}"> </li> </ul> <vt-button-bar class="context-actions" buttons="{data.buttonList}"> </vt-button-bar>', '', '', function(opts) {
    var tag = this;
