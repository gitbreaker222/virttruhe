...
riot.tag2('vt-demo', '<div if="{demoMethod()}"> <input type="button" onclick="{clickFn}"> <span each="{demoProp2.length}" class="{focus: true,                   item: demoMethod(this)}"> {demoProp2} </span> </div>', '', '', function(opts) {
        var tag = this;
...
riot.tag2('vt-intro-page', '<div> scan the start card </div> <vt-button-bar buttons="{buttonList}"> </vt-button-bar>', '', '', function(opts) {
    var tag = this;
...
window.onload = function () {
  ...
  app.one('finishedPreparing', initialize);
